<template>
  <div class="container">
    <Header title="Requisição - Conferência" />
    ...
    <div class="content-wrapper" v-if="!loading && requisicao">
      ...
      <h3 class="items-title">Itens para Conferência</h3>
    ...
    <div v-if="loading" class="loading-message">Carregando detalhes da conferência...</div>
    
    <footer class="footer-actions">
        <button @click="goBack" class="button-secondary">Voltar</button>
        <button @click="efetivarConferencia" class="button-primary" :disabled="isSaving">
            {{ isSaving ? 'Aguarde...' : 'Efetivar Conferência' }}
        </button>
    </footer>
    </div>
    </main>
...
async function efetivarConferencia() {
  ...
  try {
    ...
    acaoConcluida.title = 'Conferência Efetivada!';
    acaoConcluida.message = 'A requisição foi enviada com sucesso para a próxima etapa de embalagem.';
